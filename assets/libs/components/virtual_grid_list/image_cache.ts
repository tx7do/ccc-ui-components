...
export class ImageCache extends Component {
    ...
    public async loadImage(uri: string, callback?: Function, bundleName: string = 'resources') {
        ...
        if (!this._imgLoading) {
            ...
            await this.loopLoadImage(list, this._loadImgDuration, imgMap, bundleName);
        }
    }
     ...
     * @param bundleName
     */
    private async loopLoadImage(list: LoadTaskItemArray, duration: number, imgMap: Texture2DMap, bundleName: string = 'resources') {
        // 执行之前，先记录开始时间
        const startTime = new Date().getTime();
        while (list.length > 0) {

            let data = list.pop();
            await this.loadSingleImage(data.uri, data.cb, imgMap, bundleName);

            // 每执行完一段小代码段，都检查一下是否已经超过我们分配的本帧，这些小代码端的最大可执行时间
            if (new Date().getTime() - startTime > duration) {
                // 如果超过了，那么本帧就不在执行，开定时器，让下一帧再执行
                this.scheduleOnce(() => {
                    this.loopLoadImage(list, duration, imgMap, bundleName);
                });
                return;
            }
        }
     ...
     * @param bundleName
     */
    private async loadSingleImage(uri: string, callback: Function, imgMap: Texture2DMap, bundleName: string = 'resources') {
        let [spriteFrame, err] = await resLoader.asyncLoad<SpriteFrame>(bundleName, uri + '/spriteFrame', SpriteFrame);
        if (err) {
            error('image cache load single sprite frame failed, err:' + err);
            return false;
        }
