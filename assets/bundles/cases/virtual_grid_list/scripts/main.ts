...
import {VirtualGridList} from "db://assets/libs/components/virtual_grid_list/virtual_grid_list";
...
export class VirtualGridListScene extends Component {
    ...
    protected showList(pageNo: number, itemCount?: number) {
        ...
        for (let i = (pageNo - 1) * itemCount + 1; i <= total; i++) {
            picIndex++;
            const item: DataItem = {
                pic: "textures/avatar/avatar_" + picIndex,
                date: "3/9 12:00",
    ...
    onBtnUpdateItem_Tap() {
        ...
        for (let i = 0; i < list.length && i < 5; i++) {
            itemData = list[i];
            let picId = 29 - i;
            itemData.pic = "textures/avatar/avatar_" + picId;
